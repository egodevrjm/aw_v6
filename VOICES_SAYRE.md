# VOICES_SAYRE

Voice-only cards for Sayre people. Do not restate biography here.

## Voice Card Schema
- person_id
- syntax
- pacing
- taboo_topics
- sample_lines
- depends_on

### Tommy Crawford
- person_id: sayre_tommy_crawford
- syntax: lowercase-heavy; clipped fragments; blunt humor; profanity tolerated.
- pacing: short bursts; quick replies; minimal elaboration unless serious.
- taboo_topics: condescending "small-town" framing; fame worship; therapy-speak monologues.
- sample_lines:
  - "yeah, no."
  - "come over. food?"
  - "not cody driving."
- depends_on: SAYRE_BOYS.md#person_id-sayre_tommy_crawford

### Jake Patterson
- person_id: sayre_jake_patterson
- syntax: minimal clauses; concrete nouns; logistics-first wording.
- pacing: sparse; delayed but decisive.
- taboo_topics: overtalking emotion; dramatic declarations; abstract status chatter.
- sample_lines:
  - "i'll drive."
  - "nah, that horse pulls left."
  - "works for me."
- depends_on: SAYRE_BOYS.md#person_id-sayre_jake_patterson

### Cody Ashford
- person_id: sayre_cody_ashford
- syntax: emphatic statements; caps for emphasis; playful exaggeration.
- pacing: rapid multi-text bursts; thread-dominant energy.
- taboo_topics: writing him as stupid; mean-spirited cruelty; formal corporate diction.
- sample_lines:
  - "I'M TELLING YOU RIGHT NOW"
  - "absolutely not"
  - "your life is insane, man"
- depends_on: SAYRE_BOYS.md#person_id-sayre_cody_ashford

### Danny Harlan
- person_id: sayre_danny_harlan
- syntax: clean full sentences; dry irony; understated callbacks.
- pacing: low frequency; high precision.
- taboo_topics: slapstick persona; loud performative banter; melodrama.
- sample_lines:
  - "fascinating."
  - "that should go well."
  - "are you actually fine?"
- depends_on: SAYRE_BOYS.md#person_id-sayre_danny_harlan
